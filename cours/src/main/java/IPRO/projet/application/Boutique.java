...
public class Boutique implements IGestion{
    ...
    public Client getClientFromUsername(String username){
        ...
        while(!found && i<clients.size()){
            if(clients.get(i).getUsername().getUsername().equals(username))
                found = true;
    ...
    public ISellable getArticleFromName(String name){
        Boolean found = false;
        int i=0;

        while(!found && i<stock.getStockQteList().size()){

            if(stock.getStockQteList().get(i).getIsellable().getNameObject().getName().equals(name))
                found = true;

            i++;

        }
        return stock.getStockQteList().get(i-1).getIsellable();
